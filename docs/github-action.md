...
The workflow runs on pull requests, emits inline GitHub annotations, and writes a Markdown report to GitHub's step summary. To also maintain one persistent report comment on each PR, add `--pr-comment "$PR_NUMBER"` and give the workflow `issues: write` permission.
